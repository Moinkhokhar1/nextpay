...
class _SendScreenState extends State<SendScreen> {
  ...
  // ── Fetch wallet balance from GET /wallet ───────────────────────
    ...
    try {
      ...
      setState(() {
        _balance = double.tryParse(
            response.data['balance']?.toString() ?? '0') ?? 0.0;
        _lockedBalance = double.tryParse(
            response.data['locked_balance']?.toString() ?? '0') ?? 0.0;
        _balanceLoading = false;
      });
    } catch (e) {
      // Offline — use in-memory wallet from provider instead of resetting to 0
      debugPrint("WALLET LOAD OFFLINE: using local balance");
  ...
  // ADD these imports at the top of send_screen.dart if not already there:
// import 'package:connectivity_plus/connectivity_plus.dart';
// import '../offline/transaction_engine.dart';
// import '../offline/wallet_engine.dart';
  ...
  Future<void> _handleSend() async {
    ...
    if (isOnline) {
      ...
      try {
        final response = await ApiService.instance.post("/wallet/transfer", data: {
        ...
        if (response.data['success'] == true) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(response.data['message'] ?? 'Transfer failed')),
          );
        }
      } catch (e) {
        debugPrint("ONLINE SEND ERROR: $e");
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Transfer failed. Check your connection.')),
        );
      }
    } else {
      // ── OFFLINE: lock balance + save transaction locally ─────────
      try {
        final auth = context.read<AuthProvider>();
        ...
        if (result['success'] == true) {
          // Refresh UI balance (locked balance updated)
          _loadWallet();
          setState(() {
            _successAmount = numericAmount.toStringAsFixed(2);
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(result['message'] ?? 'Offline transaction failed')),
          );
        }
      } catch (e) {
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: const BoxDecoration(
                          ...
                          border: Border(
                            top: BorderSide(color: Color(0xFFC85A1E), width: 2),
                            right: BorderSide(color: Color(0xFFC85A1E), width: 2),
                            bottom: BorderSide(color: Color(0xFFC85A1E), width: 2),
                            left: BorderSide(color: Color(0xFFC85A1E), width: 5),
                          ),
                        ),
                        child: const Text(
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 20),
                      // TODO: QrImageView(data: _confirmationQR!, size: 220)
                      const SizedBox(height: 16),
                      Text(
