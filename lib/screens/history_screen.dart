...
//
//     final totalReceived =
//     received.fold<num>(0, (s, tx) => s + tx.amount);
...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  // NOTE: No initState-based listener registration here anymore.
  // We rely on context.watch<AuthProvider>() inside build(), which is
  // automatically subscribed/unsubscribed by Provider in sync with this
  // widget's lifecycle. This avoids the dangling-listener crash seen in
  // the logs (AuthProvider.notifyListeners -> _onAuthChange -> context.read
  // on an unmounted state) and the resulting "stuck on loading" bug.
    ...
    if (!_hasLoaded) {
      ...
      if (currentUserId != null) {
        WidgetsBinding.instance
            .addPostFrameCallback((_) => _loadTransactions());
      } else {
        _loading = false;
      }
    }
